...
let RATE_CACHE = {
    at: 0,
    value: null,     // number
    inflight: null,  // Promise<number> (parallel requestlarni bitta qiladi)
};

const RATE_TTL_MS = 5 * 60 * 60 * 1000;
...
class b1HANA {
    ...
    leads = async (req, res, next) => {
        try {
            ...
            if (meetingDateStart || meetingDateEnd) {
                ...
                let end = meetingDateEnd ? moment.tz(meetingDateEnd, ['DD.MM.YYYY', 'YYYY-MM-DD'], tz).endOf('day').toDate() : null;
                ...
                if (Object.keys(dateQuery).length > 0) {
                    if (meeting === 'lastUpdatedDate') {
                        addAndCondition(filter, { newTime: { $exists: true, $ne: null, ...dateQuery } });
                    } else if (meeting === 'createdAt') {
                        addAndCondition(filter, { time: dateQuery });
                    } else if (meeting === 'time') {
                        addAndCondition(filter, {
                            $or: [{ newTime: dateQuery }, { newTime: null, time: dateQuery }]
                        });
        ...
        try {
            ...
            if (missing.length) {
                return res.status(400).json({
                    ...
                    location: 'telegram_bot_lead_required_fields',
                });
            }
            ...
            if (!cleanedPhone) {
                return res.status(400).json({
                    ...
                    location: 'telegram_bot_lead_invalid_phone',
                });
            }
            ...
            async function uploadLeadImages({ files, lead, cardCode }) {
                ...
                for (const file of files) {
                    try {
                        ...
                        const savedImage = await LeadImage.create(imagePayload);

                        uploadedImages.push(savedImage);
                        uploadResults.push({
                            fileName: file.originalname,
                            success: true,
                            imageId: savedImage?._id?.toString() || null,
                            error: null,
                        });
                    } catch (uploadErr) {
                        ...
                        await writeLeadEvent({
                            ...
                            message: `Image upload failed: ${file.originalname}`,
                            changes: [],
                        });
                    }
                }
            ...
            if (lead) {
                ...
                if (files.length) {
                    const uploadResponse = await uploadLeadImages({
                        files,
                        lead,
                        cardCode: lead.cardCode ?? null,
                    });

                    savedImages = uploadResponse.uploadedImages;
                    imageUploadResults = uploadResponse.uploadResults;
                }
                ...
                await writeLeadEvent({
                    ...
                    message: files.length
                        ? 'Existing lead bumped and updated with images (Telegram bot)'
                ...
                return res.status(200).json({
                    message: files.length
                        ? 'Existing lead updated and bumped successfully (Telegram bot) with images'
                    ...
                    ...(files.length ? { imageUploadResults } : {}),
                });
            }
            ...
            await writeLeadEvent({
                ...
                action: 'lead_created',
                message: files.length
                    ? 'Lead created with images (Telegram bot)'
            ...
            return res.status(201).json({
                message: files.length
                    ? 'Lead created successfully (Telegram bot) with images'
    ...
    calcInternalScore({
                          ...
                          totalAmountUzs   // ✅ NEW: UZS (sumPen uchun)
                      }) {
        if (score == null || score === '') return null;

        const B_USD = Number(totalAmount) || 0;
        const C_USD = Number(totalPaid) || 0;
        ...
        // paidRatio USD bo'yicha
        const paid = B_USD > 0 ? C_USD / B_USD : 0;
        const overRate = B_USD > 0 ? D / B_USD : 0;
        ...
        // ✅ sumPen UZS bo‘yicha
        const B_UZS = Number(totalAmountUzs) || 0;
        let sumPen = 0;
        if (B_UZS <= 5_000_000) sumPen = 30;
        else if (B_UZS <= 10_000_000) sumPen = 25;
        else if (B_UZS <= 15_000_000) sumPen = 20;
        else if (B_UZS <= 20_000_000) sumPen = 10;
        else sumPen = 0;

        // ✅ MUHIM: sumPen ayirilishi shart
    ...
    calculateLeadPaymentScore = async (cardCode) => {
        ...
        const usdUzsRate = await this.getUsdUzsRateCached(() =>
            this.fetchRateFromDb(this.execute.bind(this), DataRepositories)
        );

        const totalAmountUzs = totalAmount * usdUzsRate;
    ...
    updateLead = async (req, res, next) => {
        try {
            ...
            function parseRecallDate(v) {
                const m = moment(v, 'DD.MM.YYYY HH:mm', true); // strict
                if (!m.isValid()) return null; // yoki error tashlang
                return m.toDate();
            }
            ...
            if (bodyRecall) {
                ...
                if (!parsed) {
                    return res.status(400).json({
                        message: "recallDate format invalid. Expected 'DD.MM.YYYY HH:mm'",
                        location: 'recallDate',
                    });
                }
                if (keepRecallStatuses.has(nextStatus)) {
                    validData.recallDate = parsed;
                } else {
                    return res.status(400).json({
                        message: `recallDate can only be set for statuses: ${Array.from(keepRecallStatuses).join(', ')}`,
                        location: 'recallDate',
                    });
                }
            }
    ...
    async getUsdUzsRateCached(fetchRateFn) {
        ...
        // cache valid
        if (RATE_CACHE.value != null && (now - RATE_CACHE.at) < RATE_TTL_MS) {
            return RATE_CACHE.value;
        }

        // agar hozir rate olayotgan bo'lsa, shuni kutamiz
        if (RATE_CACHE.inflight) {
            return RATE_CACHE.inflight;
        }

        RATE_CACHE.inflight = (async () => {
            const rate = await fetchRateFn(); // number qaytishi kerak
            const n = Number(rate);

            if (!Number.isFinite(n) || n <= 0) {
                // fallback: agar eskisi bo'lsa, o'shani ishlat
                if (RATE_CACHE.value != null) return RATE_CACHE.value;
                throw new Error(`Invalid USD->UZS rate: ${rate}`);
            }

            RATE_CACHE.value = n;
            RATE_CACHE.at = Date.now();
            return n;
        })();

        try {
            return await RATE_CACHE.inflight;
        } finally {
